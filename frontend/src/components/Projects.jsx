  ...
  return (
    <section className="relative w-full bg-slate-50 py-24 px-6 min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-6 animate__animated animate__fadeInUp animate__delay-1s">
          {projects.map((project, index) => (
            <div key={index} className="w-full group relative rounded-3xl bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgba(79,70,229,0.12)] hover:-translate-y-2 transition-all duration-500 overflow-hidden flex flex-col h-full">
              ...
              <div className="relative w-full h-32 sm:h-36 overflow-hidden bg-slate-100 p-3">
              ...
              {/* Card Body */}
              <div className="p-3 sm:p-4 flex flex-col flex-grow relative z-10 bg-white/40">
                {/* Title */}
                <h3 className="text-sm sm:text-base font-extrabold text-slate-900 line-clamp-2 mb-1.5 group-hover:text-indigo-600 transition-colors leading-tight">
                  {project.title}
                </h3>
                
                {/* Description */}
                <p className="text-slate-500 font-medium leading-relaxed text-[11px] sm:text-xs line-clamp-3 mb-3">
                  {project.desc}
                </p>
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {project.tech.slice(0, 2).map((tech, i) => (
                    <span key={i} className="px-1.5 py-0.5 bg-slate-100 text-slate-600 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider rounded border border-slate-200/50">
                      {tech}
                    </span>
                  ))}
                  {project.tech.length > 2 && (
                    <span className="px-1.5 py-0.5 bg-slate-50 text-slate-400 text-[9px] font-bold rounded">+{project.tech.length - 2}</span>
                  )}
                </div>

                {/* Action Buttons (Bottom) */}
                <div className="flex items-center gap-1.5 mt-auto pt-3 border-t border-slate-100">
                  <Link to={`/project/${project.id}`} className="flex-1 flex items-center justify-center gap-1 bg-slate-900 text-white px-2 py-1.5 sm:px-3 sm:py-2 rounded-lg text-[10px] sm:text-[11px] font-bold shadow-md hover:bg-indigo-600 hover:shadow-[0_0_20px_-5px_rgba(79,70,229,0.5)] transition-all duration-300">
                    Details <ArrowRight size={11} />
                  </Link>
                  <a href={project.github} target="_blank" rel="noreferrer" className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 bg-white text-slate-700 border border-slate-200 rounded-lg hover:border-indigo-600 hover:text-indigo-600 hover:bg-indigo-50 transition-all duration-300 shadow-sm" title="Source Code">
                    <FaGithub size={13} />
                  </a>
                  {project.liveLink && (
                    <a href={project.liveLink} target="_blank" rel="noreferrer" className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 bg-white text-emerald-600 border border-emerald-200 rounded-lg hover:border-emerald-500 hover:bg-emerald-50 transition-all duration-300 shadow-sm" title="Live Site">
